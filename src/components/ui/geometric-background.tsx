...
interface GeometricBackgroundProps {
    ...
    isInverse?: boolean;
    isMask?: boolean;
    maskBaseColor?: string;
    fillColor?: string;
    initialFillColor?: string;
    strokeColor?: string;
    triggerRange?: [number, number]; // [start, end]
    showStrokes?: boolean;
}
...
function TransitionBlock({
    ...
    const [rangeStart, rangeEnd] = triggerRange;

    // Spread the transition over the trigger range based on random order
    const blockDuration = (rangeEnd - rangeStart) * 0.15; // Each block takes 15% of the total range time
    const blockStart = rangeStart + (order / total) * (rangeEnd - rangeStart - blockDuration);
    const blockEnd = blockStart + blockDuration;

    // Color transition: from initial color to target color
    // If no initial color is provided, we just use the final color
    const fill = useTransform(
        progress,
        [rangeStart, blockStart, blockEnd],
        initialFillColor ? [initialFillColor, initialFillColor, fillColor] : [fillColor, fillColor, fillColor]
    );

    // If we are in mask mode, the blocks are always "black" (to create holes)
    const finalFill = isMask ? "black" : fill;

    // If we have an initial color, opacity is always 1 (solid color swap)
    // Otherwise, we use the existing fade in/out logic
    const opacityValues = initialFillColor ? [1, 1, 1] : (isInverse ? [1, 1, 0] : [0, 0, 1]);
...
export default function GeometricBackground({
    ...
    // Generate a unique, safe mask ID (avoiding React's colon-containing IDs for SVG href stability)
    const rawId = useId();
    const maskId = `geo-mask-${rawId.replace(/:/g, '')}`;

    const randomOrder = useMemo(() =>
        Array.from({ length: GEOMETRIC_PATHS.length }, (_, i) => i).sort(() => Math.random() - 0.5)
        , []);
            ...
            <div className="relative w-[5085px] h-[2750px] flex-shrink-0">
                ...
                >
                    {isMask && (
                        <defs>
                            <mask id={maskId}>
                                <rect x="0" y="0" width="5085" height="2750" fill="white" />
                                {GEOMETRIC_PATHS.map((path, index) => (
                                    <TransitionBlock
                                        key={`mask-fill-${index}`}
                                        path={path}
                                        index={index}
                                        total={GEOMETRIC_PATHS.length}
                    ...
                    {isMask ? (
                        ...
                        <>
                            {/* Background wireframe layer - uses the 'opacity' prop */}
                            {(showStrokes || true) && GEOMETRIC_PATHS.map((path, index) => (
                                <motion.path
                                    key={`wire-${index}`}
                                    d={path}
                                    stroke={strokeColor}
                            ...
                            {/* Animated fill layer - always solid */}
                            {GEOMETRIC_PATHS.map((path, index) => (
                                <TransitionBlock
                                    key={`fill-${index}`}
                                    path={path}
