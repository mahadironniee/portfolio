...
export default function SecondSection() {
    ...
    // Track full visibility range for parallax (from entry to exit)
    const { scrollYProgress: parallaxProgress } = useScroll({
        target: sectionRef,
        offset: ["start end", "end start"],
    });

    // Background animation synced with scroll for continuous parallax
    const backgroundX = useTransform(parallaxProgress, [0, 1], ["-10%", "10%"]);
    const backgroundY = useTransform(parallaxProgress, [0, 1], ["0%", "5%"]);
